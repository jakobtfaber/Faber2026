% !! GENERATED FILE -- do not edit by hand. Values live in
%    analysis/foregrounds/census/foreground_table_data.json; markup in foreground_table_emitter.py.
%    Regenerate: uv --directory analysis run --frozen python -m foregrounds.propagation.foreground_table_emitter --out ../foreground_table.tex
%    Object IDs + verdicts are cross-checked against data/intervening_census_registry.csv.
% Foreground census table. Values from the analysis foreground validation
% (LS DR9 / DESI DR1 / NED / PS1-STRM / GLADE+ / WHL12); see sec:obs-fg.
% The budgeted cluster comes from WenHan2024. One additional WHL12 cluster is
% tabulated as a census systematic but lacks the adopted M500/R500 inputs.
% Only confirmed and inconclusive systems are tabulated; the 7 candidates
% refuted as background (and stellar classifications) are described in the text
% (sec:obs-fg), matching the treatment of the other categorical cuts.
\startlongtable
\begin{deluxetable*}{lllrrlllll}
\tabletypesize{\tiny}
\tablecaption{Intervening foreground halos and clusters along the sightlines to the 12 CHIME/DSA co-detected FRBs, validated against DESI Legacy DR9 (Zhou+2021 photo-z), DESI DR1 spec-z, NED, PS1-STRM, GLADE+, and WHL12. Verdicts: \emph{confirmed} (catalog $z<z_{\rm host}$) and \emph{inconclusive} ($z$ within $1\sigma$ of host, host $z$ unknown, or no trustworthy $z$). Discovery-stage redshifts that fail the validation hierarchy are shown only as flagged context and do not enter the dispersion-measure budget. Candidates refuted as background are omitted (Section~\ref{sec:obs-fg}). \label{tab:foreground}}
\tablehead{\colhead{Burst} & \colhead{Type} & \colhead{Obj ID} & \colhead{$b$} & \colhead{$b/R_{500}$} & \colhead{$z$} & \colhead{$z$ src} & \colhead{Class} & \colhead{Verdict} & \colhead{Note}}
\startdata
FRB 20220207C & halo & 195373100910393540 & 76 & \nodata & $0.469\pm0.047$ & PS1-STRM phot & GALAXY & inconclusive & phot.\ extrap. \\
FRB 20220310F & halo & 1473 & 101 & \nodata & $0.358\pm0.113$ & LS/Zhou phot & REX & confirmed & LS/Zhou phot < host \\
 & halo & 1582 & 184 & \nodata & $0.471\pm0.190$ & LS/Zhou phot & EXP & inconclusive & within 1$\sigma$ \\
FRB 20220506D & halo & 195393180643665627 & 74 & \nodata & \nodata & PS1-STRM & UNSURE & inconclusive & STRM unsure \\
 & halo & WISEA J211150.32+724807.8 & 208 & \nodata & $0.084$ & WISExSCOS phot & galaxy cand. & inconclusive & low-quality photo-z \\
FRB 20221113A & halo & WISEA J044538.83+701843.3 & 16 & \nodata & \nodata & GLADE+ dist. & galaxy cand. & inconclusive & no validated z \\
FRB 20221203A & halo & 194453151328186646 & 232 & \nodata & \nodata & PS1-STRM & UNSURE & inconclusive & STRM unsure \\
FRB 20230307A & cluster & J115120.4+714435, 1254337 & 604 & 0.83 & 0.200 & DESI spec & cluster & confirmed & DESI spec < host \\
 & cluster & WHL J115048.0+714428 & 614 & \nodata & 0.189 & WHL12/NED & cluster & confirmed & no WH24 $M_{500}$ \\
 & halo & 194051777813062524, 1190 & 105 & \nodata & 0.110 & DESI spec & DEV & confirmed & DESI spec < host \\
 & halo & 1072 & 105 & \nodata & $0.300\pm0.079$ & LS/Zhou phot & EXP & inconclusive & within 1$\sigma$ \\
 & halo & 194041777780157594, 1153 & 130 & \nodata & 0.215 & DESI spec & REX & confirmed & DESI spec < host \\
 & halo & 983 & 116 & \nodata & $0.165\pm0.059$ & LS/Zhou phot & EXP & confirmed & LS/Zhou phot < host \\
 & halo & 194021777634832653, 832 & 144 & \nodata & $0.193\pm0.028$ & LS/Zhou phot & DEV & confirmed & LS/Zhou phot < host \\
 & halo & 986 & 203 & \nodata & $0.208\pm0.067$ & LS/Zhou phot & REX & inconclusive & within 1$\sigma$ \\
 & halo & 953 & 194 & \nodata & 0.199 & DESI spec & REX & confirmed & DESI spec < host \\
FRB 20230325A & halo & 197030881733398302 & 60 & \nodata & $0.305\pm0.068$ & PS1-STRM phot & GALAXY & inconclusive & host z unknown \\
 & halo & 197040882212782495 & 234 & \nodata & $0.618\pm0.126$ & PS1-STRM phot & GALAXY & inconclusive & host z unknown \\
FRB 20230814B & \nodata & \nodata & \nodata & \nodata & \nodata & \nodata & \nodata & no candidates & outside deep footprints \\
FRB 20230913A & halo & 192963050359413614 & 136 & \nodata & $0.308\pm0.073$ & PS1-STRM phot & GALAXY & inconclusive & within 1$\sigma$ \\
 & halo & 192943050854547067 & 209 & \nodata & \nodata & PS1-STRM & UNSURE & inconclusive & STRM unsure \\
FRB 20240122A & \nodata & \nodata & \nodata & \nodata & \nodata & \nodata & \nodata & no candidates & outside deep footprints \\
FRB 20240203A & halo & 196723126173351736 & 104 & \nodata & $0.475\pm0.044$ & PS1-STRM phot & GALAXY & inconclusive & phot.\ extrap. \\
 & halo & 196673126794497004 & 111 & \nodata & \nodata & PS1-STRM & UNSURE & inconclusive & STRM unsure \\
 & halo & 196733128040225775 & 270 & \nodata & 0.054 & NED & galaxy & inconclusive & NED PUN photo-z lacks uncertainty \\
FRB 20240229A & halo & 192821700026167542, 824 & 177 & \nodata & 0.203 & DESI spec & SER & confirmed & DESI spec < host \\
 & halo & 192831699797402822, 827 & 243 & \nodata & $0.240\pm0.012$ & LS/Zhou phot & SER & confirmed & LS/Zhou phot < host \\
 & halo & 825 & 281 & \nodata & $0.348\pm0.203$ & LS/Zhou phot & REX & inconclusive & within 1$\sigma$ \\
\enddata
\tablecomments{All 26 tabulated physical systems exist in $\geq$1 public catalog. Adopted redshifts follow the DESI/Legacy/NED hierarchy; the two flagged discovery-stage galaxy redshifts lack the quality or classification needed for adoption. Five confirmed systems recovered under two catalog identifiers (sub-arcsecond coincidence at identical redshift) are shown once, with both identifiers on the row. $b$ is the proper impact parameter (kpc) at the object redshift; for confirmed systems it is uniformly recomputed from the burst position, object coordinates, and adopted redshift, while inconclusive systems retain their discovery-stage values. The candidates refuted as background by the validation stage (five physical systems) are omitted, as are candidates spectroscopically classified as stars (Section~\ref{sec:obs-fg}). The budgeted cluster is drawn from the \citet{WenHan2024} DESI Legacy/WISE catalog and passes within $R_{500}$. WHL J115048.0$+$714428 is a confirmed WHL12 cluster at comparable impact parameter but is absent from the adopted Wen--Han 2024 source and has no adopted $M_{500}$ or $R_{500}$; it is tabulated as an unmodeled one-sided systematic and contributes zero to the reported $\mathrm{DM_{int}}$. Fourteen further Wen--Han foreground clusters at $b>R_{500}$ are omitted. Sightlines with no tabulated system (FRB~20230814B and FRB~20240122A) are listed explicitly; their intervening term is unconstrained rather than measured to be zero (Table~\ref{tab:budget}, note~u).}
\end{deluxetable*}
