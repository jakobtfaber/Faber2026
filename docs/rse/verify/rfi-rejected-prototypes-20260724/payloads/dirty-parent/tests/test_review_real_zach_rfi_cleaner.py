...
def test_nominal_coarse_frequency_keeps_fully_masked_group_coordinates():
    frequency = np.arange(8, dtype=float)

    coarse_frequency = MODULE.nominal_coarse_frequency(frequency, factor=4)

    np.testing.assert_allclose(coarse_frequency, [1.5, 5.5])
...
def test_robust_row_labels_call_it_a_component_not_a_cleaner():
    labels = MODULE.candidate_labels("robust_rows")

    assert "component" in labels["warning"].lower()
    assert "cleaner" not in labels["warning"].lower()
    assert "robust" in labels["short"].lower()

    seed_labels = MODULE.candidate_labels("offpulse_seed_rows")
    assert "component" in seed_labels["warning"].lower()
    assert "cleaner" not in seed_labels["warning"].lower()
    description = MODULE.candidate_method_description("offpulse_seed_rows")
    assert description["protected_on_pulse"] is True
    assert description["minimum_seed_rows_per_group"] == 4

    voltage_labels = MODULE.candidate_labels(
        "offpulse_seed_plus_voltage_kurtosis"
    )
    assert "voltage" in voltage_labels["short"].lower()
    voltage_description = MODULE.candidate_method_description(
        "offpulse_seed_plus_voltage_kurtosis"
    )
    assert voltage_description["protected_on_pulse"] is True
    assert voltage_description["voltage_kurtosis_sigma"] == 5.0


def test_owner_marked_intervals_are_summarized_without_becoming_configuration():
    frequency = np.array([610.0, 620.0, 710.0, 720.0])
    spectrum = np.array([1.0, 9.0, 2.0, 12.0])
    summaries = MODULE.owner_marked_interval_summaries(
        spectrum, frequency, np.ones(4, dtype=bool)
    )

    assert set(summaries) == {"600_650_mhz", "700_750_mhz"}
    assert summaries["600_650_mhz"]["maximum"] == 9.0
    assert summaries["700_750_mhz"]["maximum"] == 12.0


def test_robust_row_candidate_broadcasts_only_selected_rows(monkeypatch):
    training = np.zeros((3, 2), dtype=np.float32)
    context = np.arange(12, dtype=np.float32).reshape(3, 4)
    combined = np.concatenate((training, context), axis=1)
    selected = np.array([False, True, False])

    def fake_row_mask(values, row_valid, interval, **kwargs):
        assert interval == (0, training.shape[1])
        np.testing.assert_array_equal(values, combined)
        return selected, {"standard_deviation": np.arange(3), "lag1_correlation": np.arange(3)}

    candidate, mask, rows, _ = MODULE.apply_candidate(
        combined,
        np.ones(3, dtype=bool),
        (0, training.shape[1]),
        "robust_rows",
        robust_mask_function=fake_row_mask,
    )

    np.testing.assert_array_equal(rows, selected)
    np.testing.assert_array_equal(mask, selected[:, None] & np.isfinite(combined))
    assert np.isnan(candidate[1]).all()
    np.testing.assert_array_equal(candidate[[0, 2]], combined[[0, 2]])


def test_voltage_allowed_support_uses_only_approved_blocks_and_excludes_burst():
    allowed = MODULE.voltage_allowed_support(305 * 128, np.array([0, 48]))

    assert allowed[:, 60 * 128].all()
    assert not allowed[:, 140 * 128].any()
    assert not allowed[0, 230 * 128]
    assert allowed[1, 230 * 128]
    assert allowed[0, 200 * 128]
    assert not allowed[1, 200 * 128]
