...
**Status — fallback research only.** The active manuscript bridge uses GitHub
Sync. The native-Git design was abandoned after the analysis split; do not
activate it unless the two-gitlink GitHub Sync test fails and the owner chooses
this fallback. The findings below remain the contract for that contingency.
