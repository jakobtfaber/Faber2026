...
scattering screen rather than a Milky-Way origin. Finally, we show that both unmodeled
temporal sub-components and an unphysical band-dependent pulse-broadening function
can bias the single-component scattering index by up to several units; re-fitting
the sample under a single physical pulse-broadening function with per-band
multiplicity, the well-constrained sightlines yield stable indices, with the final
per-sightline adjudication in progress.
\end{abstract}
