#!/bin/bash
# PostToolUse hook: the every-10-minutes trigger for ACTIVE sessions.
# Fires after every tool call; when the last journal entry is >=10 min
...
echo "$now_s" > "$NAG"
printf '{"hookSpecificOutput":{"hookEventName":"PostToolUse","additionalContext":"[journal] %sm since last entry — append NOW, mid-turn: analysis/scripts/journal-append.sh <agent> <lane> working \\"<what you are working on right now>\\" (10-min cadence, analysis/docs/rse/protocols/journal-protocol.md)."}}\n' "$age"
exit 0
