...
- **`make figures` is live.** It runs
  `figure_flow.py regen --manuscript --clone-ok` against
  ...
  with PyYAML, so it runs under the environment above; a bare interpreter
  ...
  data-bound (`clone_ok: false`); regenerate with
  `uv run --frozen python scripts/figure_flow.py regen --id fig1_gallery`
  (writes staging under `figure_review/artifacts/staging/`, then
  `figure_review.py new-batch` — never silent promote). The Oran DSA
  qualification PNG is **not** a manuscript
