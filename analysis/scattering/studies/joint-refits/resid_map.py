...
def panel(axrow, m, p, label, gain=True):
    ...
    axrow[0].set_title(f"{label} data")
    axrow[1].set_title("model")
    ...
    ac = np.correlate(rprof, rprof, "full")
...
def main():
    ...
    if gain:  # 8-param / shared-zeta: no c0/gamma, model = g_f * K_f(t)
        pC = FRBParams(
            c0=1.0,
            t0=p["t0_C"],
            gamma=0.0,
    ...
    for a in ax[:, :3].ravel():
        ...
        a.set_ylabel("f (GHz)")
    fig.suptitle(
        f"{b}: joint alpha={al:.2f} tau1GHz={tau:.3f}ms  | "
        f"CHIME chi2={cC[0]:.2f} lag1={cC[2]:.2f} | DSA chi2={cD[0]:.2f} lag1={cD[2]:.2f}",
        fontsize=11,
    )
    fig.tight_layout()
    fp = f"{out}/{b}_resid_map.png"
    fig.savefig(fp, dpi=120, bbox_inches="tight")
    print(
        f"{b}: CHIME chi2={cC[0]:.2f} std={cC[1]:.2f} lag1ac={cC[2]:.2f} | "
