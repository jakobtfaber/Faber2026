...
def onpulse_widths(prof, t):
    ...
    base = np.median(edge)
    sig = 1.4826 * np.median(np.abs(edge - base)) + 1e-9
    p = prof - base
    on = p > 3 * sig
    if on.sum() < 3:
...
def main():
    b = sys.argv[1]
    nsub = int(sys.argv[2]) if len(sys.argv) > 2 else 4
    ...
    for i in range(nsub):
        ...
        ax.set_title(
            f"{b} CHIME {freq[sl][0]:.3f}-{freq[sl][-1]:.3f} GHz  w_rms={w_rms:.3f} w_tail={w_tail:.3f} ms",
            fontsize=8,
        )
    axes[-1].set_xlabel("time (ms)")
    fig.tight_layout()
    fp = save_fig(fig, f"{out}/{b}_chime_subband_profiles", dpi=110)
    ...
    print(
        f"{b}: within-CHIME width-freq scaling beta (width~nu^-beta):  rms={b_rms:.2f}  tail={b_tail:.2f}"
    )
    for fcc, wr, wt in rows:
        print(f"    nu={fcc:.3f} GHz  w_rms={wr:.3f}  w_tail={wt:.3f} ms")
    print(f"    wrote {fp}")
    json.dump(
        {"burst": b, "beta_rms": b_rms, "beta_tail": b_tail, "subbands": rows.tolist()},
        open(f"{out}/{b}_within_chime.json", "w"),
        indent=2,
    )
