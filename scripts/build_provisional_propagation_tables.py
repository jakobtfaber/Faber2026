...
JOINT = ROOT / "analysis/campaigns/scattering-dm-locked-2026-07-14/results"
SCINT = ROOT / "analysis/campaigns/scintillation-dsa-lorentzian-2026-07-07/results"
...
FOREGROUND = ROOT / "analysis/campaigns/foregrounds"
...
def main():
    ...
    regenerated_ledger.write_text(json.dumps(full_payload, indent=2) + "\n")
    active_payload = {
        key: value for key, value in full_payload.items()
        if key != "foreground_alignment_rows"
    }
    active_payload["status"] = "SCREEN_ANALYSIS_PENDING"
    (OUT / "results.json").write_text(json.dumps(active_payload, indent=2) + "\n")
