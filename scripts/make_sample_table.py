...
Reads trust-safe observational inputs plus V6-revalidated association
diagnostics from the pinned analysis submodule and emits an AASTeX deluxetable.

Sources:
  - analysis/config/bursts.yaml -> MJD, UTC, RA/Dec
  - locked FLITS burst metadata -> nickname -> TNS
  - analysis/campaigns/crossmatching/toa_crossmatch_results.json
  - analysis/campaigns/crossmatching/association_report.json
...
def main() -> None:
    ...
    tex = f"""% Rows generated by scripts/make_sample_table.py from analysis inputs
% and locked FLITS source commit {commit}: config/bursts.yaml (MJD/UTC/RA/Dec),
% burst_metadata.py (nickname -> TNS), campaigns/crossmatching/
% toa_crossmatch_results.json (shared-DSA-DM TOA residuals), and
% association_report.json (P_cc/verdict).
