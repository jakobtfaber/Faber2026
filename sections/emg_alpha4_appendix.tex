...
Two corollaries govern our fit design. First, the only fixed scattering index
...
\citep{Bhat2004,Cordes2025}---rather than a turbulence spectral index. This is
...
% The algebra in this appendix -- the closure values, the divergence of s_c,
% the closed form and beta->4 limit of M(beta), the tail convergence
% conditions, the tau(nu) exponent, and the equivalence of Eq. (eq:emg) with
% the Gaussian (*) one-sided-exponential convolution and with the erfcx kernel
% implemented in pipeline/scattering/scat_analysis/burstfit.py -- was verified
% symbolically with WolframScript; script and outputs (12/12 True):
