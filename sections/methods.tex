...
For each sightline with a spectroscopic host redshift and a quality-passing
joint fit, we estimate the isotropic-equivalent burst energy directly from the
joint CHIME--DSA fit, without extrapolating either band's spectrum beyond where
it is constrained. The joint fit returns a per-band spectral amplitude and
index,
