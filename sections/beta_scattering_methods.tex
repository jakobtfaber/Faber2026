...
$\alpha=4$ and a one-sided exponential PBF; the exponential kernel is in fact
\emph{uniquely} the $\beta=4$ member of this family, so an exponentially
modified Gaussian profile presupposes $\alpha=4$
(Appendix~\ref{app:emg-alpha4}).

An immediate---and, for interpreting shallow empirical scalings, essential---%
consequence is that $\alpha=4$ is a \emph{two-sided floor} for a single thin
screen of power-law turbulence. On the shallow-spectrum branch,
$\alpha(\beta)=2\beta/(\beta-2)$ is monotonically decreasing
($d\alpha/d\beta=-4/(\beta-2)^{2}<0$), so every inertial-range member with
$\beta<4$ scales \emph{more} steeply than $\nu^{-4}$, reaching the floor only
in the $\beta\to4$ limit. On the steep-spectrum branch, $\beta\ge4$, the phase
structure function saturates to the square-law form
$D_\phi(\rho)\propto\nu^{-2}\rho^{2}$: the phase screen is smooth on the
diffractive scale, and the scattering time locks to $\tau\propto\nu^{-4}$
independent of $\beta$ \citep{Rickett1990}. Physically, the $\nu^{-2}$
chromaticity of the plasma phase plus diffraction alone already produce
$\tau\propto\nu^{-4}$; multi-scale turbulent structure can only steepen the
scaling beyond that baseline, never flatten it. No single thin screen of
power-law turbulence can therefore produce $\alpha<4$: a measured
$\alpha<4$ falsifies one of the model assumptions---a thin screen (versus an
extended or multi-screen medium), isotropy, a negligible inner scale, an
untruncated scattering disc, or scattering-free burst morphology---rather than
measuring an exotic turbulence index \citep{Bhat2004,Cordes2025}. This floor is
also why the sampled range $\beta\in[3,4)$ corresponds to
$\alpha\in(4,6]$: values of $\alpha$ below 4 are not merely disfavored by the
prior, they are unreachable within the physical family being fit.
