...
at the native band reference. The provenance of the underlying timestamps is
stated explicitly, and four layers are kept separate throughout: (i) the raw
time coordinate of each data product, (ii) the reconstruction of the trigger
or frame time from it, (iii) the statistical uncertainty of the profile fit
that defines a burst's arrival time, and (iv) the cross-observatory
absolute-clock systematic. On the DSA-110 side, the archived trigger MJD
combines an exact integer spectrum counter with a text-serialized elapsed-time
record whose six-significant-digit precision introduces a rounding of up to
$\pm5$\,ms at hour-scale acquisition times; the counter arithmetic was
validated against the surviving-row corpus and then applied to all twelve
bursts, removing this rounding to a numerical floor of about $1\,\mu$s. The residuals
tabulated here use the archived catalog timestamps and therefore inherit that
rounding. On the CHIME side, the timestamp is reconstructed from
the baseband file's internal time coordinate (native $2.56\,\mu$s sampling),
which defines a precise time coordinate on the UTC scale \emph{as kept by the
station clock}, not an independently calibrated absolute UTC epoch; the absolute
accuracy of the CHIME station clock is not independently quantified here, and
we deliberately assign it no numerical value---in particular, the native
sample interval is not a clock-accuracy statement. Consequently, the residuals
below are consistency diagnostics, not measurements with a certified absolute
timing uncertainty. Each TOA is translated to the common
...
therefore compute a candidate model-based correction to the differential
scattering bias, as described next. Adopting that correction requires the fit
to pass the numerical quality check and its diagnostic figure to be
reviewed. The current pinned products satisfy neither condition for any burst,
so the association analysis retains the observed peak-sample TOAs.
...
(Section~\ref{sec:jointfit}) define a candidate estimator. The
...
with the scattering tail deconvolved out. For the diagnostic model extension we
quantify the correction from the peak-sample TOA as the
...
The diagnostic correction is required to be non-negative and to grow with the
fitted scattering timescale. The model path also carries a posterior width on
$t_0$, the uncertainty on $\Delta_{\rm scat}$, and a component-choice ambiguity
for multi-component bursts; if the model checks pass, those terms are folded
into the per-band arrival uncertainty together with the DM-referral term and the
source-position contribution to the geometric delay. At the current pin all
twelve model fits are marked \texttt{MARGINAL} and their figures are unreviewed.
Consequently, \texttt{model\_corrected\_offset\_ms} and its extended error
budget remain diagnostic-only, while the canonical
\texttt{measured\_offset\_ms} remains byte-identical to the observed-peak field
\texttt{peak\_measured\_offset\_ms}. The manuscript-facing residuals use that
peak offset rather than the unvalidated model extension. Their legacy
normalized values use a nominal timing budget; because the inter-site
absolute-clock contribution is not independently quantified, those normalized
values are conditional diagnostics rather than certified significance
measurements.
...
inside the pre-specified association window in arrival time and sky position,
and, for the eight events with an independent CHIME-side DM constraint, in
dispersion measure as well. We quantify this with a chance-coincidence
(false-alarm) probability $P_{\rm cc}$.
...
broad windows defined below, the eight pairs whose pre-specified
...
to $\sum_i\mu_i\approx1.8\times10^{-6}$. For a class-independent bound within
the adopted rate, timing, and positional model, we assign every trial the larger
position-and-time-only value. This gives
$\sum_j\mu_j < 64(4.41\times10^{-7})=2.9\times10^{-5}$ and leaves the
conclusion unchanged.

We evaluate Eq.~\eqref{eq:pcc_mu} with deliberately broad rate, positional,
timing, and DM windows. The window choices increase the estimated chance rate,
while the eight DM-conditioned values retain dependence on the stated model for
the CHIME DM distribution. We use an all-sky CHIME rate rounded up to
$R = 10^{3}\,\mathrm{sky^{-1}\,day^{-1}}$ (above the Catalog~1 value of
$525\pm30^{+140}_{-130}\,\mathrm{sky^{-1}\,day^{-1}}$ above 5\,Jy\,ms at
600\,MHz for $\mathrm{DM}>100\,\mathrm{pc\,cm^{-3}}$ and scattering time below
10\,ms; \citealt{CHIME2021Catalog}), a
...
The adopted timing and positional windows are far wider than the millisecond
timing residuals and arcsecond-scale DSA localizations. A direct Monte Carlo,
using the same assumed DM distribution, reproduces the closed-form calculation
to $0.3\%$ in a regime where both are measurable; this checks the numerical
implementation, not the astrophysical DM model. The analytic result is stable
across several orders of magnitude of variation in each window and in $R$.
...
$\sum_i\mu_i\approx1.8\times10^{-5}$ (and the class-independent 64-trigger
bound to $<2.9\times10^{-4}$), leaving the association verdict unchanged.
...
of the known repeater FRB~20220912A, and one co-detected counterpart,
FRB~20230814B, has a single earlier DSA-110-only detection ($47$\,d prior, same
beam declination and redshift $z=0.5535$; the earlier burst's raw data were not
retained). No repeat of any co-detected source lands within the $\pm1$\,s
association gate, and the earlier FRB~20230814B detection is already counted as
a separate trial in the $N=64$ chance-coincidence denominator. Because
repetition enters Eq.~\eqref{eq:pcc_mu} only through the effective CHIME rate
inside the $\pm1$\,s gate, even a repeater-driven rate enhancement of $10^{3}$
leaves $\sum_i\mu_i\lesssim10^{-2}$.
...
per-burst systematic from the unquantified inter-site clock/timestamp
alignment, the archived DSA trigger-time rounding described above, and, for
the eight originally DM-conditioned pairs, the residual dispersive delay left
by the CHIME--DSA DM offset. With the new phase-coherence measurements, the
largest band offset is
$0.117\,\mathrm{pc\,cm^{-3}}$ (FRB~20221113A), corresponding to approximately
$2.8$\,ms when re-referenced between 1.5\,GHz and 400\,MHz,
a convention-dependent term reported separately rather than folded in. The
legacy table combines the shared-DM CHIME+DSA arrival error, intrinsic pulse
width, and a nominal $1$\,ms clock term. Under that assumption all twelve
residuals ($-2.7$ to $+8.4$\,ms) fall within the displayed $3\sigma_i$ range,
with a largest normalized residual of $2.6\sigma$ (FRB~20221203A). These
normalized values do not include a measured CHIME absolute-clock uncertainty
and therefore must not be interpreted as formal absolute-timing
significances.
...
residual is $+2.1\pm0.9$\,ms under that same nominal budget. The quoted
$2.4\sigma$ displacement from zero excludes the unquantified absolute-clock
systematic and is therefore not evidence for a measured clock offset. A
constant-offset model is only mildly preferred over zero offset
($\chi^{2}=8.9/11$ versus $14.7/12$); we retain the unsubtracted residuals in
Table~\ref{tab:sample} and treat the weighted mean only as a candidate
inter-site calibration term for future investigation. We stress
...
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{figures/toa_offset_decomposition.pdf}
    ...
    geometric delay is the vertical tick, and the filled circle is the observed
    peak-sample offset. The grey connector is the residual (offset minus
    ...
    the full $\sim\!11\,$ms sample spread corresponds to only
    $\approx1.2\,\mathrm{pc\,cm^{-3}}$. The vertical line at zero separates the
...
the order. The sign is instead set by the residual, whose $\sim\!11\,$ms
sample spread corresponds to only $\approx1.2\,\mathrm{pc\,cm^{-3}}$ of
...
largest referenced offset ($6.2\,$ms for FRB~20220506D) by more than two orders
of magnitude, so a sign flip of a few milliseconds is invisible to
$P_{\rm cc}$. The shared $-2.17\,$ms pedestal is not an \emph{independent}
association signature---a position-matched chance interloper, lying within the
same $0.5^{\circ}$ gate, has essentially the same DRAO--OVRO projection and
would carry the same pedestal (the geometric delay varies by only
$\lesssim0.04\,$ms across the gate). What the pedestal provides is a
\emph{consistency} check: the measured offsets sit on the geometric prediction
to within the per-burst budget, so no pair shows an anomalous residual that
would flag a mis-association or an uncorrected instrumental delay.
