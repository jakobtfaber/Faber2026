...
We likewise defer a foreground--propagation cross-match. The foreground census
is validated, but the available scattering values predate the production
pulse-broadening model and cannot provide a current observed broadening scale.
We therefore report no foreground fraction, causal screen assignment, or
fit--budget tension from those values. Such comparisons require certified
scattering fits and, for screen consistency, the fixed-index refits described
above; survey-coverage censoring must also remain explicit.
...
%
% (3) Host redshift. Three provisional-z rows (FRBs 20230814B, 20230913A,
% 20240203A):
