...
The two-screen calculation is not yet evaluable under the accepted dual-$\tau$
policy (Table~\ref{tab:twoscreen-provisional}). Scintillation consistency must
pair each DSA component with the single-exponential $\alpha$ fixed to 4
$\tau_{\rm consistency}$ refit, not with the free-$\alpha$ joint-fit track used
for the morphology and citable-$\alpha$ audit. Those consistency refits are not
present in the frozen catalog. We therefore report the seven otherwise
eligible sightlines as pending rather than computing products or screen
verdicts from the wrong $\tau$ track. Once those refits exist, the planned
calculation will evaluate $\tau_{\rm consistency}(\nu)\Delta\nu_d$ separately
at every clean narrow DSA component frequency, without imposing a DSA
bandwidth power law, and will compare it with the conservative common-screen
interval $0.1$--$2$. The mostly uncalibrated DSA widths and unavailable full
posterior covariance remain additional gates. No two-screen, screen-distance,
or Galactic/host identity inference is made from the current products.

\input{twoscreen_provisional_table}

We next cross-match the independent fitted-scattering and foreground budgets
against the registry-authoritative V4 foreground census
...
model. If a future policy-compliant two-screen result supports multiple
propagation layers, this foreground complex would be a candidate scattering
screen; the present data neither assign a scintillation screen to it nor
determine a screen distance.
